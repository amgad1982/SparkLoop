...
export const PodAudioStage: React.FC<PodAudioStageProps> = ({
  ...
  const sizeConfig = useMemo(() => {
    ...
    if (forceCompact) {
      return {
        ...
        cardWidth: 'min-w-[52px] max-w-[62px]',
        nameText: 'text-[9px]',
    ...
    if (count <= 2) {
      ...
      return {
        ...
        cardWidth: 'min-w-[78px] max-w-[90px]',
        nameText: 'text-[11px]',
        badgeText: 'text-[9px]',
    ...
    if (count <= 5) {
      ...
      return {
        ...
        cardWidth: 'min-w-[64px] max-w-[74px]',
        nameText: 'text-[10px]',
    ...
    if (count <= 8) {
      ...
      return {
        ...
        innerRadius: 'rounded-[10px]',
        cardWidth: 'min-w-[54px] max-w-[62px]',
        nameText: 'text-[9px]',
        badgeText: 'text-[8px]',
    ...
    return {
      ...
      cardWidth: 'min-w-[44px] max-w-[50px]',
      nameText: 'text-[8.5px]',
      badgeText: 'text-[7.5px]',
  ...
  return (
    ...
    >
      {/* 1. Header Row (Always rendered at minimal clean height) */}
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-2 min-w-0">
          ...
          {/* When collapsed and speakers exist, show mini avatar cluster inline in header! */}
        ...
        <div className="flex items-center gap-1.5">
          ...
          {/* Density Toggle Button (When 3+ speakers present) */}
          {hasSpeakers && !isCollapsed && (
            <Tooltip
              content={
                forceCompact
                  ? isArabic
                    ? 'عرض الأيقونات بالحجم القياسي'
                    : 'Standard icon size'
                  : isArabic
                  ? 'تصغير حجم أيقونات المتحدثين'
                  : 'Minimize speaker icons'
              }
              position="bottom"
            >
              <button
                ...
                className={`p-1.5 rounded-xl border text-xs transition-all shadow-sm ${
                  forceCompact
                    ...
                    : 'bg-zinc-100 dark:bg-zinc-900 text-zinc-500 dark:text-zinc-400 border-zinc-200 dark:border-zinc-800 hover:text-zinc-800 dark:hover:text-zinc-200'
                }`}
              >
                {forceCompact ? (
          ...
          {/* Collapse/Expand Toggle Button (When speakers present) */}
          {hasSpeakers && (
            <Tooltip
              content={
                isCollapsed
            ...
            >
              <button
                ...
                className="p-1.5 rounded-xl bg-zinc-100 dark:bg-zinc-900 hover:bg-zinc-200 dark:hover:bg-zinc-800 border border-zinc-200 dark:border-zinc-800 text-zinc-500 dark:text-zinc-400 hover:text-zinc-800 dark:hover:text-zinc-200 transition-colors shadow-sm"
              >
                {isCollapsed ? (
            ...
            <div className="flex items-center gap-1.5">
              ...
              >
                <button
                  ...
                  className={`p-1.5 rounded-xl border text-xs font-bold flex items-center gap-1 transition-all shadow-sm ${
                    isHandRaised
                      ? 'bg-amber-500/20 text-amber-700 dark:text-amber-300 border-amber-500/40 shadow-md animate-pulse'
                      : 'bg-zinc-100 dark:bg-zinc-900 text-zinc-600 dark:text-zinc-400 border-zinc-200 dark:border-zinc-800 hover:text-amber-600 dark:hover:text-amber-400'
                  }`}
                >
              ...
              <Tooltip content={isArabic ? 'الانضمام للمسرح والتحدث المباشر' : 'Join live stage to speak'} position="bottom">
                <button
                  type="button"
                  onClick={onJoinStage}
                  className="px-3 py-1.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-md shadow-emerald-600/25 transition-all active:scale-95 shrink-0"
                >
            ...
            <div className="flex items-center gap-1.5">
              ...
              <Tooltip content={isArabic ? 'مغادرة المسرح والنزول للجمهور' : 'Leave stage and return to listeners'} position="bottom">
                <button
                  ...
                  className="px-2.5 py-1.5 bg-zinc-100 dark:bg-zinc-900 hover:bg-zinc-200 dark:hover:bg-zinc-800 text-zinc-700 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-800 rounded-xl text-xs font-semibold transition-colors shadow-sm"
                >
      ...
      {/* 3. Speakers Row Container (Only rendered/expanded when speakers exist and not collapsed) */}
      <AnimatePresence>
        {hasSpeakers && !isCollapsed && (
          <motion.div
          ...
          >
            ...
            {canScrollLeft && (
              <button
                ...
                className="absolute left-0 top-1/2 -translate-y-1/2 z-20 p-1 rounded-full bg-white/90 dark:bg-zinc-900/90 border border-zinc-300 dark:border-zinc-700 shadow-md text-zinc-700 dark:text-zinc-200 hover:scale-110 transition-transform"
                title="Scroll left"
              >
            ...
            {canScrollRight && (
              <button
                type="button"
                onClick={() => scrollBy(isArabic ? -160 : 160)}
                className="absolute right-0 top-1/2 -translate-y-1/2 z-20 p-1 rounded-full bg-white/90 dark:bg-zinc-900/90 border border-zinc-300 dark:border-zinc-700 shadow-md text-zinc-700 dark:text-zinc-200 hover:scale-110 transition-transform"
                title="Scroll right"
              >
              ...
              {speakers.map((speaker) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`flex flex-col items-center gap-1 shrink-0 ${sizeConfig.cardWidth}`}
                  >
      ...
      {isOnStage && !isMuted && (
        <div className="flex items-center gap-2.5 pt-1.5 border-t border-zinc-200 dark:border-zinc-800/60 text-[10px] text-zinc-600 dark:text-zinc-400">
          ...
          <span className="font-mono text-zinc-700 dark:text-zinc-400 font-bold shrink-0">{micLevel}%</span>
        </div>
      )}
    </motion.div>
  );
